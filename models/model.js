...
module.exports = {
  ...
  addCategory: function (category) {
    ...
    if (categories.includes(category)) throw Error("La categoría ya existe");
  ...
  addReview: function (name, stars, text, user) {
    ...
    let FindProduct = products.find((el) => el.name === name)
        if(!FindProduct){
        ...
        FindProduct.reviews.push({stars, text, user});
        FindProduct.rating = FindProduct.reviews.reduce((acc, el) => acc+el.stars, 0)/ FindProduct.reviews.length
  ...
  getReviews: function (name) {
    ...
    if(!products.find((el) => el.name === name))
    throw Error("Producto no encontrado");
  ...
  getRating: function (name) {
    ...
    // Si no recibe parámetros (name) devuelve sólo el nombre de los 5 productos mejor puntuados, ordenados de mayor a menor puntaje
     ...
     if(!name) return products.sort((a, b) => b.rating - a.rating ).map((el) => el.name).slice(0, 5)
